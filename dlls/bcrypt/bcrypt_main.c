...
{
    TRACE( "%p, %s, %p, %lu, %#lx\n", key, debugstr_w(prop), value, size, flags );
...
NTSTATUS WINAPI BCryptExportKey(
    BCRYPT_KEY_HANDLE export_key_handle,
    ...
    struct key *export_key = get_key_object(export_key_handle);
    struct key *encrypt_key;
    ULONG required_size, padded_len;
    UCHAR *padded_input;
    NTSTATUS status = STATUS_SUCCESS;
    symmetric_key aes;
    unsigned long wrapped_len;

    TRACE("%p, %p, %s, %p, %lu, %p, %#lx\n", export_key_handle, encrypt_key_handle,
          debugstr_w(type), output, output_len, size, flags);

    if (!export_key) return STATUS_INVALID_HANDLE;
    ...
    return key_export(export_key, type, output, output_len, size);
}
